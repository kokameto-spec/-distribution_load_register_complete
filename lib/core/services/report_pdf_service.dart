...
class ReportPdfService {
  ...
  static const int _distributorsPerPage = 12;
  ...
  static Future<Uint8List> buildPdf({
    ...
    if (allDistributorsHourly) {
      ...
      final activeDistributors =
          distributors.where((item) => item.active).toList()
            ..sort((a, b) => a.name.compareTo(b.name));

      final groups = _splitDistributorsForPages(activeDistributors);
      ...
      for (final day in _daysBetween(start, end)) {
        for (final pageDistributors in groups) {
          document.addPage(
            pw.Page(
              pageFormat: PdfPageFormat.a4.landscape,
              margin: const pw.EdgeInsets.fromLTRB(9, 7, 9, 7),
              theme: theme,
              build: (_) => _buildAllDistributorsPage(
                day: day,
                hour: hour,
                index: index,
      ...
      final distributorName =
          distributor?.name ??
          (records.isNotEmpty ? records.first.distributorName : '');

      document.addPage(
        pw.MultiPage(
          pageFormat: PdfPageFormat.a4.landscape,
          margin: const pw.EdgeInsets.fromLTRB(12, 8, 12, 8),
          theme: theme,
          header: (_) => _buildHeader(
            logoBytes: logoBytes,
            reportTitle: 'أحمال موزع',
            distributorName: distributorName,
          ),
          footer: (_) => _buildCompactFooter(),
  ...
  static pw.Widget _buildHeader({
    ...
    final title = distributorName != null &&
            distributorName.trim().isNotEmpty
        ? '$reportTitle: ${distributorName.trim()}'
        : reportTitle;
    ...
    return pw.Directionality(
      ...
      child: pw.Container(
        ...
        child: pw.Row(
          ...
          children: [
            // يمين الصفحة: بيانات الشركة كل بند في سطر منفصل.
            pw.SizedBox(
              width: 220,
              child: pw.Column(
            ...
            // المنتصف: أحمال موزع + اسم الموزع في نفس السطر.
            pw.Expanded(
              child: pw.Center(
                child: pw.Text(
                  title,
            ...
            // الشمال: اسم البرنامج واضح + اللوجو.
            pw.SizedBox(
              width: 180,
  ...
  static pw.Widget _buildAllDistributorsPage({
    ...
    final firstEnd = distributors.length > _distributorsPerTable
        ? _distributorsPerTable
        : distributors.length;

    final firstGroup = distributors.sublist(0, firstEnd);

    final secondGroup = distributors.length > _distributorsPerTable
        ? distributors.sublist(_distributorsPerTable)
        : <Distributor>[];
  ...
  static List<pw.Widget> _buildSingleDistributor({
    ...
    final distributorId =
        distributor?.id ?? sorted.first.distributorId;
    ...
    return <pw.Widget>[
      ...
      // إصلاح اتجاه العنوان الذي كان يظهر مقلوبًا.
      pw.Directionality(
        textDirection: pw.TextDirection.rtl,
        child: pw.Align(
      ...
      pw.Directionality(
        ...
        child: pw.Table(
          ...
          children: [
            pw.TableRow(
              children: [
                _smallHeader('التاريخ'),
                _smallHeader('الوقت'),
                _smallHeader('المدخل'),
                _smallHeader('الإجمالي'),
                for (final cell in cellNumbers) _smallHeader('$cell'),
              ],
            ),
            for (final record in sorted)
              pw.TableRow(
                children: [
                  _smallValue(_formatDate(record.recordedAt)),
                  _smallValue(
                    DateFormat('HH:mm').format(record.recordedAt),
                  ),
                  _smallValue(record.operatorName),
                  _smallValue(
                    record.totalLoad.toStringAsFixed(1),
                  ),
                  for (final cell in cellNumbers)
                    _smallValue(
                      (record.cellValues[cell] ?? 0)
                          .toStringAsFixed(1),
                    ),
                ],
              ),
    ...
    return pw.Directionality(
      ...
      child: pw.Container(
        ...
        child: pw.Row(
          ...
          children: [
            _compactSignature('مدخل البيانات'),
            _compactSignature('إعداد'),
            _compactSignature('مدير التشغيل'),
            _compactSignature('اعتماد المدير العام'),
          ],
        ),
      ),
    );
  ...
  static pw.Widget _compactSignature(String title) {
    return pw.Row(
      children: [
        pw.Text(
          '$title: ',
  ...
  static pw.Widget _smallHeader(String text) {
    return pw.Padding(
  ...
  static pw.Widget _smallValue(String text) {
    return pw.Padding(
  ...
  static List<List<Distributor>> _splitDistributorsForPages(
    List<Distributor> distributors,
  ) {
    final result = <List<Distributor>>[];

    for (var start = 0;
        start < distributors.length;
        start += _distributorsPerPage) {
      final end = start + _distributorsPerPage <
              distributors.length
          ? start + _distributorsPerPage
          : distributors.length;

      result.add(distributors.sublist(start, end));
    }
  ...
  static String _effectiveFileName(String? value) {
    ...
    return name.isEmpty ? 'A7mal_ElMoz3at' : name;
  }
...
class _ReportPdfIndex {
  ...
  factory _ReportPdfIndex.build(
    ...
    for (final record in records) {
      ...
      // بدون الساعة لمنع ظهور "لم يسجل" رغم وجود السجل.
      final latestKey =
      ...
      for (final entry in record.cellValues.entries) {
        final key =
            '${record.distributorId}|${entry.key}';
  ...
  String minimumValue({
    ...
    final value =
        ranges['$distributorId|$cellNumber']?.minimum;
  ...
  String maximumValue({
    ...
    final value =
        ranges['$distributorId|$cellNumber']?.maximum;
