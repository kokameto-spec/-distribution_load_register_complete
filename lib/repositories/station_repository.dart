...
class StationRepository {
  ...
  Stream<List<Station>> watchAll() {
    if (_windows) {
      ...
      ).asyncMap((_) => _getAllRest()).startWith(_getAllRest());
    }

    return _stations.snapshots().map((snapshot) {
      final stations =
          snapshot.docs.map(Station.fromFirestore).toList();
  ...
  Future<List<Station>> _getAllRest() async {
    ...
    final stations = docs.map((doc) {
      ...
      return Station(
        ...
        createdAt:
            _date(data['createdAt']) ?? DateTime.now(),
    ...
    if (_windows) {
      return FirebaseRestService.createDocument(
        ...
        data: {
          ...
          'active': true,
          'transformers':
              transformers.map((e) => e.toMap()).toList(),
  ...
  Future<void> update({
    ...
    await _stations.doc(id.trim()).update({
      'name': name.trim(),
      'active': active,
      'transformers':
          transformers.map((e) => e.toMap()).toList(),
  ...
  Future<void> delete(String id) async {
    ...
    await _stations.doc(id.trim()).delete();
  }
...
extension _StationStreamStart<T> on Stream<T> {
