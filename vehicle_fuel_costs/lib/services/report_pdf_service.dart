...
class ReportPdfService {
  static Future<Uint8List> fuelingPages(List<FuelingRecord> records) async {
    ...
    final logo = pw.MemoryImage(
      (await rootBundle.load('assets/company_logo.png')).buffer.asUint8List(),
    );

    for (final record in records) {
      final rawImages = await FirebaseService.instance.fuelingImages(record.id);
      final images = <pw.MemoryImage>[
        for (final bytes in rawImages.take(4)) pw.MemoryImage(bytes),
      ];
              ...
              children: [
                ...
                pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(
                      'عداد السيارة: ${NumberFormat.decimalPattern().format(record.odometer)} كم',
                      style: const pw.TextStyle(fontSize: 10),
                    ),
                    pw.Text(
                      '${AppConfig.approvalTitle}: ........................',
                      style: const pw.TextStyle(fontSize: 10),
                    ),
                  ],
                ),
  ...
  static Future<Uint8List> consumptionTable(
    List<ConsumptionEntry> rows,
    String monthKey,
  ) async {
    final doc = pw.Document();
    final regular = await PdfGoogleFonts.cairoRegular();
        ...
        build: (_) => [
          pw.Directionality(
            ...
            child: pw.Column(
              children: [
                pw.Text(
                  '${AppConfig.companyName} - ${AppConfig.departmentName}',
  ...
  static const List<int> _transparentPng = [
    ...
    68, 65, 84, 8, 29, 99, 96, 96, 96, 0, 0, 0, 5, 0, 1, 13, 10, 45, 180,
