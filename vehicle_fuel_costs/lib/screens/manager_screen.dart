...
class _ManagerScreenState extends State<ManagerScreen> {
  ...
  Future<void> _saveFuelingPdf(List<FuelingRecord> records) async {
    try {
      ...
      _msg('تم الحفظ بنجاح: $path');
    } catch (e) {
      _msg('تعذر الحفظ: $e');
    }
  }

  Future<void> _printFuelingPdf(List<FuelingRecord> records) async {
    try {
  ...
  Future<void> _shareFuelingPdf(List<FuelingRecord> records) async {
    try {
      final bytes = await ReportPdfService.fuelingPages(records);
  ...
  Future<void> _editFueling(FuelingRecord record) async {
                ...
                Wrap(
                  ...
                  children: List.generate(4, (i) {
                    ...
                    return OutlinedButton.icon(
                      onPressed: () async {
                        final picked = await _picker.pickImage(source: ImageSource.gallery, maxWidth: 1024, maxHeight: 1024, imageQuality: 55);
    ...
    if (ok == true) {
      ...
      if (newOdo == null || name.text.trim().isEmpty) {
        _msg('أدخل اسم السائق وقراءة عداد صحيحة.');
        return;
      }
      try {
        ...
        _msg('تم تعديل التموينة وإعادة حساب كشف الاستهلاك.');
      } catch (e) {
        _msg('تعذر التعديل: $e');
      }
    }
  ...
  Future<void> _deleteFueling(FuelingRecord record) async {
    final ok = await showDialog<bool>(
          ...
          builder: (_) => AlertDialog(
            ...
            content: Text('سيتم حذف التموينة والصور وقراءة العداد المرتبطة بالسيارة ${record.vehicleNumber}. هل تريد المتابعة؟'),
            actions: [
              TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('إلغاء')),
    ...
    try {
      ...
      _msg('تم الحذف وإعادة حساب القراءات التالية.');
    } catch (e) {
      _msg('تعذر الحذف: $e');
    }
  }
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Wrap(spacing: 6, children: [
                    IconButton(onPressed: () => _saveFuelingPdf(records), tooltip: 'حفظ PDF', icon: const Icon(Icons.save_alt)),
                    IconButton(onPressed: () => _printFuelingPdf(records), tooltip: 'طباعة', icon: const Icon(Icons.print)),
                    IconButton(onPressed: () => _shareFuelingPdf(records), tooltip: 'مشاركة', icon: const Icon(Icons.share)),
                  ]),
                ]),
              ),
                      ...
                      Card(
                        child: Padding(
                          ...
                          child: Row(children: [
                            ...
                            const Spacer(),
                            IconButton(onPressed: () => _editFueling(record), tooltip: 'تعديل / استبدال صور', icon: const Icon(Icons.edit)),
                            IconButton(onPressed: () => _saveFuelingPdf([record]), tooltip: 'حفظ', icon: const Icon(Icons.save_alt)),
                            IconButton(onPressed: () => _printFuelingPdf([record]), tooltip: 'طباعة', icon: const Icon(Icons.print)),
                            IconButton(onPressed: () => _shareFuelingPdf([record]), tooltip: 'مشاركة', icon: const Icon(Icons.share)),
  ...
  Future<void> _editConsumption(ConsumptionEntry entry) async {
    ...
    try {
      ...
      _msg('تم تعديل القراءة وإعادة حساب المسافات.');
    } catch (e) {
              ...
              Align(
                ...
                child: Padding(
                  ...
                  child: Wrap(spacing: 6, children: [
                    ...
                    IconButton(
                      tooltip: 'طباعة',
                      onPressed: () => Printing.layoutPdf(onLayout: (_) => ReportPdfService.consumptionTable(rows, _monthKey), name: 'كشف_الاستهلاك_$_monthKey.pdf'),
...
class _VehiclesAdminState extends State<_VehiclesAdmin> {
  ...
  Widget build(BuildContext context) => Column(children: [
        Padding(padding: const EdgeInsets.all(12), child: Align(alignment: Alignment.centerLeft, child: FilledButton.icon(onPressed: () => _edit(), icon: const Icon(Icons.add), label: const Text('إضافة سيارة')))),
        Expanded(
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(height: 8),
                TextField(controller: password, obscureText: true, decoration: InputDecoration(labelText: existing == null ? 'كلمة المرور' : 'كلمة مرور جديدة (اتركها فارغة دون تغيير)')),
                const SizedBox(height: 8),
                DropdownButtonFormField<String>(
                  initialValue: role,
                  decoration: const InputDecoration(labelText: 'نوع المستخدم'),
                  items: const [DropdownMenuItem(value: 'driver', child: Text('سائق')), DropdownMenuItem(value: 'manager', child: Text('مدير'))],
                ...
                builder: (_, snap) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final car = vehicles.where((v) => v.code == user.vehicleCode).cast<Vehicle?>().firstOrNull;
                      return ListTile(
                        ...
                        trailing: IconButton(onPressed: () => _editUser(vehicles, user), icon: const Icon(Icons.edit)),
                      );
                    },
                  );
                },
